...
# WHY THIS EXISTS
# ---------------
# Issue #4, "Get rid of the need to run the container in privileged mode", has
# been open since March 2023. Nobody has answered it here, and nobody has
# answered it anywhere else either: every published Dell OMSA container image
...
# is the shape of an answer to issue #4, while "everything works" and "nothing
...
# a running Dell_OMSA container; it publishes no port; and it only ever runs
...
# Lower case, because Docker Hub requires it : a repository name with a capital in it is
# refused as "invalid reference format", so 'tigerblue77/Dell_OMSA' is not a repository that
# exists anywhere. The publishing workflow derives this name by stripping '_Docker' from
# 'tigerblue77/Dell_OMSA_Docker' and lower-casing the result, which is what Docker Hub
# actually holds -- confirmed against the registry, 'latest', amd64, and the same image on
# ghcr.io/tigerblue77/dell_omsa
readonly DEFAULT_IMAGE='tigerblue77/dell_omsa'
readonly CONTAINER_NAME_PREFIX='omsa-privilege-probe'
readonly CONTAINER_LABEL='com.github.tigerblue77.omsa-privilege-probe'
...
table ready to paste into issue #4.
...
rung_rationale() {
	...
	09-rawio-ipmi-megaraid) printf '%s\n' 'SYS_RAWIO with the PERC ioctl node passed in as a device. This is the rung that could plausibly answer issue #4 for storage without any bind mount at all.' ;;
...
# Dell_OMSA container the operator is probably already running. It takes the
...
build_run_argv() {
	...
	# probe run never fights the operator's real Dell_OMSA container for it.
	argv=(docker run --detach
		--name "$container_name"
		--label "${CONTAINER_LABEL}=${RUN_ID}"
		--env "OMSA_username=${OMSA_USERNAME}"
		--env "OMSA_password=${password_value}")
...
print_caveats() {
	...
	out '* **systemd in a container is its own confound.** This image runs `/sbin/init` as PID 1, and systemd itself is one of the things `--privileged` makes easy. A rung where systemd never comes up has not shown that OMSA needs that privilege; it has shown that *this packaging* does. Running OMSA'"'"'s daemons directly, without systemd, is a separate experiment and a plausible other half of the answer to issue #4.'
	out "* **Port ${OMSA_WEB_PORT} is probed from inside the container**, because publishing it would collide with the real Dell_OMSA container this host may be running. A pass means the web server answered, not that the host can reach it; publishing a port is orthogonal to privilege and needs no measurement."
