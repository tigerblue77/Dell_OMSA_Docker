...
# Automated test suite for the Dell OMSA Docker image.
#
# Everything runs against mocked system commands (tests/mocks/) and a throwaway
# sandbox root, so the suite needs no Dell hardware, no OMSA installation, no
# Docker and no network : bash, coreutils, GNU grep and awk are enough. Run it
# with :
#
#   ./tests/run_tests.sh                 # run everything
#   ./tests/run_tests.sh --list          # list the test cases without running them
#   ./tests/run_tests.sh -f credential   # run the cases whose name, or whose case file, matches
...
# happens to be on. A case that builds a repository under a temporary directory and
# configures an identity in it locally is not covered on its own : git has THREE sources
# above that, and only two of them are files.
#
...
# config, so a single variable rewrites what those sandboxes answer. Not hypothetical :
# Claude Code on the web sets three such pairs, two of them
# "url.https://github.com/.insteadOf", which is what made a case asserting the SSH form of
# a remote pass in the sibling repository without ever reaching the branch written for it.
#
...
# Load the helpers. There is nothing else to source here : the code under test is
# a container entrypoint rather than a library, so it is run as a process, from
# the sandboxed copy lib/harness.sh builds
...
# "test_the_user_is_created" -> "the user is created"
function humanize_test_case_name() {
  local -r NAME="${1#test_}"
  printf '%s' "${NAME//_/ }"
}

# tests/cases/20_credential_validation.sh -> "credential validation"
...
for TEST_FILE in "${TEST_FILES[@]}"; do
  while IFS= read -r TEST_CASE_NAME; do
    ...
    # selects. Under the name "git clone" gives this project, "-f OMSA" would match every
    # case file's path and run all of them
...
if $TAP_OUTPUT; then
  ...
  printf '%sDell OMSA Docker image - automated test suite%s\n' "$COLOR_BOLD" "$COLOR_RESET"
